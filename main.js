	...
	$("#board tr td").click(function () {
		if ($(this).text() == "" && play) {
			...
			if (checkForWinner() != -1 && checkForWinner() != "") {
				if (checkForWinner() == "X") {
					$('body').append('<div class="winner"><span>Winner</span>X</div><button onclick="location.reload();" id="reload">Restart</button>');
					$('.winner').css('background-color', 'white');
					$('#reload').css('color','black');
				} else {
					$('body').append('<div class="winner"><span>Winner</span>O</div><button onclick="location.reload();" id="reload">Restart</button>');
					$('.winner').css('background-color', 'white');
